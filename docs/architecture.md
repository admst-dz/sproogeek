...
flowchart LR
    ...
    Frontend -->|"MJPEG frames / JSON input"| Renderer
...
Renderer также обслуживает интерактивные cloud-3D сессии. Модель, Three.js и WebGL работают внутри Chromium-контейнера; пользовательский браузер получает постоянный MJPEG-поток и отправляет назад только события указателя, колесо и JSON-конфигурацию. Публичный путь `/cloud-render/*` проксируется Nginx к приватному `/cloud/*` renderer-сервиса без буферизации.

Если renderer занят или недоступен, frontend динамически загружает прежний локальный WebGL viewport. Поэтому отказ cloud-режима не блокирует конфигуратор. Главная страница использует кэшируемые серверные poster-кадры и не загружает `.glb` на устройство пользователя.

Поток cloud-3D:

1. Frontend создаёт сессию через `POST /cloud-render/session`.
2. Renderer поднимает отдельную вкладку Chromium, применяет конфигурацию и загружает модель внутри Docker-сети.
3. `GET /cloud-render/session/{id}/stream` держит открытый MJPEG-поток.
4. Pointer/wheel события идут в `POST .../input`, изменения товара — в `POST .../config`.
5. При уходе со страницы frontend удаляет сессию; зависшие сессии закрываются по TTL.
